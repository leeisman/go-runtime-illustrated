...
GC 的目的不是「讓你不用管記憶體」，而是把記憶體回收變成一套可預期的背景協作。
這一章從三色標記開始，拆到 write barrier、pacer 與常見 leak，讓你知道什麼東西會讓清潔隊變慢。

---

## 1. 第一樂章：隱喻 (The Metaphor - The Invisible Cleaner)

回到皇家圖書館的 **大黑板 / 公共區 (Heap Memory)**。
館長們 (Goroutines) 不斷在黑板上貼新的任務資料與物件 (Objects)。有些物件還被任務單指著，有些物件早就沒人再看。
如果沒人清理那些失去引用的舊資料，大黑板很快就會被貼滿，最後爆炸 (OOM)。

於是，Go 聘請了一位 **「隱形清潔工 (GC)」**。
但他有一個幾乎不可能的任務：**他必須在館長們還在工作時，一邊判斷哪些黑板內容已經沒人需要。**

*   **傳統做法 (STW - Stop The World)**:
    *   清潔工大吼一聲：「全部不准動！」(Stop)。
    *   所有館長停格。清潔工慢慢掃完。大家繼續工作。
    *   **缺點**: 服務停擺，客戶不爽 (Latency 高)。

*   **Go 的做法 (Concurrent Mark & Sweep)**:
    *   清潔工戴著耳機，在大黑板旁默默貼標籤 (Mark)。
    *   館長們繼續執行任務 (Concurrent)。
    *   **挑戰**: 如果清潔工剛確認某個物件是「垃圾」，下一秒某位館長又把它接回任務單上怎麼辦？
...
## 6. 第六樂章：極簡記憶法：GC 運行四部曲 (Executive Summary)

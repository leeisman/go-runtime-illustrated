...
### 3.1 Sequence & ACK
*   **Byte Numbering (Byte 編號)**: TCP 是給 **每個 Byte** 編號，而不是每個 Packet。這讓 TCP 能輕鬆處理封包碎片化 (Fragmentation)。
    *   **實作原理 (High Efficiency)**: 這不代表每個 Byte 身上都貼著標籤 (那太浪費空間了)。
    *   **Header Only**: TCP 封包的 Header 只記錄 **「起始號碼 (Start Sequence)」** (佔 4 Bytes)。
    *   **推算**: 如果 Header 寫 `Seq=1000`，且 payload 長度是 500 bytes。那 Kernel 就自動知道這顆包裝著 `1000 ~ 1499` 的資料。
*   **ACK (Acknowledgement)**: 接收方告訴發送方：「我收到這之前的所有資料了」。
*   **極限與解法 (PAWS)**:
